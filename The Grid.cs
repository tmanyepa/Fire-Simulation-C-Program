...
namespace FireSimulation
{
    class Grid()
    {
        ...
        {
            //Initialise the 2D array to beginning state Tree and empty space
                ...
                while(bFireRunning)
                {
                    ...
                    for(int i = 0; i < 21; ++i)
                    {
                        for(int j = 0; j < 21; ++j)
                        {
                            ...
                            {
                                // There is at least 1 tree burning, we set bNotBurning
                                bNotBurning = false;
                            }
                        }
                    }
            ...
            public void ApplySpread()
            {
               ...
               for(int i = 0; i < 21; ++i)
               {
                   for(int j = 0; j < 21; ++i)
                   {
                       ...
                       {
                           // If neighbour is on fire create fire status for this
                           ...
                           if((i - 1 > 0 && map[i-1,j] == 'X')
                             ...
                             || (j + 1 < 21 && map[i,j+1] == 'X'))
                            {
                                //If neighbour of a cell is burning get the burning state of this cell using a random generator
